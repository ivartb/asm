...
public:
    ...
    timer(epoll_wrapper &epp, type_t &side, proxy_server &proxyServer) : side(side),
        fd(timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK | TFD_CLOEXEC)),
        event(epp, fd, EPOLLIN, [this, &side, &proxyServer](uint32_t) {
    ...
    void reset() {
        ...
        if (ret) {
            throw_my_error("Failed to set timeout");
        }
    };
